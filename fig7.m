...
% Date: 9/1/2020
% Description:  This script generates the Figures 7a and 7b in the paper
...
%%% Plot MLF method
BOUND = zeros(cmax,1);
for i = 1:3 %order of Lyapu = 2c
    c = 2;
   [P,gammma] = commonLyap(c,A1,A2,B1,C1,n, div(i));
   BOUND(i)=gammma^(1/(2*c));
end
...
% Plot bound alphaat P1 level
boundu = y + BOUND(1)*exp(-div(1)*time)';
boundl = y - BOUND(1)*exp(-div(1)*time)';
plot(time, boundu, 'Color', [1,  0, 0], 'LineWidth', 1.5);
plot(time, boundl, 'Color', [1,  0, 0], 'LineWidth', 1.5);

% Plot bound alpha = at P2 level
boundu = y + BOUND(2)*exp(-div(2)*time)';
boundl = y - BOUND(2)*exp(-div(2)*time)';
plot(time, boundu, 'Color', [1,  .5, 0], 'LineWidth', 1.5);
plot(time, boundl, 'Color', [1,  .5, 0], 'LineWidth', 1.5);

% Plot bound alpha at P2 level
boundu = y + BOUND(3)*exp(-div(3)*time)';
boundl = y - BOUND(3)*exp(-div(3)*time)';
...
%matlab2tikz('F7a.tikz', 'width', '6cm', 'height', '4cm')

%%
%%%%%%%%%%%%
% Plot Figure 7b: (0 < t < 12)
%%%%%%%%%%%
T = 12;
dt = .2;
time = 0:dt:T;
...
% Plot bound alpha at P1 level
boundu = y + BOUND(1)*exp(-div(1)*time)';
boundl = y - BOUND(1)*exp(-div(1)*time)';
cutoff_time = 11.2;
num = cutoff_time/dt;
boundl = boundl(1:cutoff_time/dt);

plot(time(1:size(boundu, 1)), boundu, 'Color', [1,  0, 0], 'LineWidth', 1.5);
plot(time(1:size(boundl, 1)), boundl, 'Color', [1,  0, 0], 'LineWidth', 1.5);

% Plot bound alpha = at P2 level
boundu = y + BOUND(2)*exp(-div(2)*time)';
cutoff_time = 9;
boundu = boundu(1:cutoff_time/dt);
boundl = y - BOUND(2)*exp(-div(2)*time)';
cutoff_time = 6.5;
boundl = boundl(1:cutoff_time/dt);

plot(time(1:size(boundu, 1)), boundu, 'Color', [1,  .5, 0], 'LineWidth', 1.5);
plot(time(1:size(boundl, 1)), boundl, 'Color', [1,  .5, 0], 'LineWidth', 1.5);
...
cutoff_time = 5;
boundl = boundl(1:cutoff_time/dt);
plot(time(1:size(boundu, 1)), boundu, 'Color', [.2,  .5, 0], 'LineWidth', 1.5);
plot(time(1:size(boundl, 1)), boundl, 'Color', [.2,  .5, 0], 'LineWidth', 1.5);
...
%matlab2tikz('F7b.tikz', 'width', '6cm', 'height', '4cm')
